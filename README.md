...
| 管理ポート | OOB 管理用。パネルの左端に MGMT 区画として置く (`Gi0/0`、`mgmt0`、`fxp0` など) |
...
Catalyst の `GigabitEthernet0/0` のように、名前の階層では本体と別シャーシに
なってしまう管理ポートは、「本体と呼べる大きさのパネルがあり、そこから外れた
ポートが数本だけ」という形から判断して MGMT 区画にまとめます
(独立した「ユニット 0」パネルを作らないため)。
  ...
  - id: switch-2f
    ...
    port_layout:
      ...
      categories:          # 区分の強制指定 (ifIndex または ifName)
        "49": stack
        Gi0/0: physical    # 管理ポートではなくデータポートだった場合
```

指定できる区分は `physical` / `uplink` / `mgmt` / `lag` / `vlan` / `stack` /
`virtual` / `other` です。
